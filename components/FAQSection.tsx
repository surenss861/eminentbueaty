    ...
    <section className="py-24 bg-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {faqs.map((faq, index) => (
            <motion.div
              ...
              className="border border-luxury-blush/30 rounded-sm overflow-hidden"
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-luxury-ivory/50 transition-colors"
                aria-expanded={openIndex === index}
              >
                <span className="font-light text-luxury-navy pr-4">
                  {faq.question}
                </span>
                <ChevronDown
                  className={`w-5 h-5 text-luxury-dusty-rose flex-shrink-0 transition-transform ${
              ...
              <AnimatePresence>
                {openIndex === index && (
                  ...
                  >
                    <div className="px-6 py-4 text-luxury-charcoal/70 leading-relaxed font-light">
                      {faq.answer}
                    </div>
                  </motion.div>
                )}
        ...
        >
          ...
          <a
            href="/contact"
            className="text-luxury-dusty-rose font-medium hover:text-luxury-navy transition-colors"
          >
            Contact Us →
          </a>
        </motion.div>
